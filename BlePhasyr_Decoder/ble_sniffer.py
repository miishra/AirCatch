...
def get_samsung_privid_from_payload(payload_hex: str) -> str:
    ...
    Per captured layout (len=0x17), the PRIVID is 8 bytes starting at
    data offset 4 after the UUID (i.e., bytes data[4:12] where data[0:2] is UUID).

    Returns PRIVID as hex string (lowercase), or empty string if not found.
    """
    hx = _clean_hex(payload_hex)
    if not hx:
        return ""
    ...
    def extract_from_ad(ad_data: bytes) -> str:
        ...
        while pos + 1 < n:
            ...
            if ad_type == 0x16 and len(data) >= 12:  # need at least UUID + prefix + PRIVID
                svc_uuid = data[0] | (data[1] << 8) if len(data) >= 2 else -1
                if svc_uuid == 0xFD5A:
                    # PRIVID at data[4:12] (after UUID + 2 prefix bytes)
                    if len(data) >= 12:
...
# ============================================================
# MAC ADDRESS HIDING & SAMSUNG PRIVID CONVERSION
# ============================================================

# Global random key for MAC hiding (generated once per session)
_HMAC_KEY = None

def generate_hmac_key():
    """Generate a random 32-byte key for HMAC-based MAC hiding."""
    global _HMAC_KEY
    _HMAC_KEY = secrets.token_bytes(32)
    return _HMAC_KEY

def hide_mac_with_hmac(mac_str: str, key: bytes) -> str:
    """
    Hide a MAC address using HMAC-SHA256.
    Takes the first 6 bytes of HMAC output as the new MAC.
    
    Args:
        mac_str: MAC address as string (e.g., "AA:BB:CC:DD:EE:FF")
        key: 32-byte HMAC key
    
    Returns:
        Hidden MAC address as string in same format
    """
    if key is None:
        return mac_str
    
    # Compute HMAC-SHA256 of the MAC string
    h = hmac.new(key, mac_str.encode(), hashlib.sha256)
    hmac_digest = h.digest()
    
    # Take first 6 bytes and format as MAC
    hidden_bytes = hmac_digest[:6]
    return ":".join(f"{b:02X}" for b in hidden_bytes)

def samsung_mac_to_privid(payload_hex: str) -> str:
    """
    Extract Samsung PRIVID from payload if available.
    Returns empty string if not Samsung or PRIVID not found.
    """
    try:
        # Clean and parse
        hx = _clean_hex(payload_hex)
        ...
        # Try existing helper (covers AdvA/header cases)
        v = get_samsung_privid_from_payload(hx)
        if v:
            return v
        
        # Fallback: parse AD-only buffer
        b = _hex_to_bytes(hx)
        if not b:
            return ""
        ...
        def extract_from_ad(ad_data: bytes) -> str:
            ...
            while pos + 1 < n:
                ...
                if ad_type == 0x16 and len(data) >= 12:
                    svc_uuid = data[0] | (data[1] << 8)
                    if svc_uuid == 0xFD5A and len(data) >= 12:
                        return data[4:12].hex()
                
                pos += 1 + length
            return ""
        
        return extract_from_ad(b)
    except Exception:
        return ""

def classify_tag_type(payload_hex: str) -> str:
    """
    Classify tag ecosystem from payload.
    Returns: APPLE, SAMSUNG, GOOGLE, TILE, or UNKNOWN
    """
    try:
        hx = _clean_hex(payload_hex)
        if len(hx) < 4:
        ...
        eco = classify_tag_ecosystem_from_payload(hx)
        if eco in ("APPLE", "SAMSUNG", "GOOGLE", "TILE"):
            return eco
        return "UNKNOWN"
    except Exception:
        return "UNKNOWN"

def detect_apple_variant_and_mask(payload_hex: str, original_mac: str) -> tuple:
    """
    ...
    Returns: (ground_truth_str, masked_mac_or_original)
    """
    apple_variants = {
        "4c001219ff": ("ff", "FF:FF:FF:FF:FF:FF"),
        "4c001219fc": ("fc", "FC:FC:FC:FC:FC:FC"),
        "4c001219fd": ("fd", "FD:FD:FD:FD:FD:FD"),
        "4c001219fe": ("fe", "FE:FE:FE:FE:FE:FE"),
    }
    
    try:
        hx = _clean_hex(payload_hex).lower()
        
        for pattern, (ground_truth, masked_mac) in apple_variants.items():
            if pattern in hx:
                return ground_truth, masked_mac
...
def load_chunk_timestamp_index(csv_path: str):
    """Load chunk timing metadata produced during capture.

    Supports two formats:
      1) Legacy: chunk_index, byte_offset, chunk_size, elapsed_seconds, epoch_seconds, iso_timestamp.
      2) Mobile: chunk_index, start_byte, timestamp_ms, formatted_time, lat, lon, accuracy_m.

    Returns a lookup dict with sorted offsets; None on error.
    """
    if not csv_path:
        return None
    ...
    try:
        with open(csv_path, newline="") as f:
            ...
            for row in reader:
                # Legacy fields
                try:
                    offset = int(row.get("byte_offset", -1))
                    size = int(row.get("chunk_size", -1))
                    epoch = float(row.get("epoch_seconds", float("nan")))
                except (TypeError, ValueError):
                    offset = -1
                    size = -1
                    epoch = float("nan")

                if offset >= 0 and size > 0 and math.isfinite(epoch):
                    legacy_rows.append((offset, size, epoch, None, None, None))
                    continue

                # Mobile fields
                try:
                    m_offset = int(row.get("start_byte", -1))
                    m_epoch = float(row.get("timestamp_ms", float("nan"))) / 1000.0
                    m_lat = float(row.get("lat", float("nan")))
                    m_lon = float(row.get("lon", float("nan")))
                    m_acc = float(row.get("accuracy_m", float("nan")))
                except (TypeError, ValueError):
                    m_offset = -1
                    m_epoch = float("nan")
                    m_lat = float("nan")
                ...
                if m_offset >= 0 and math.isfinite(m_epoch):
                    mobile_rows.append((m_offset, m_epoch, m_lat, m_lon, m_acc))
    except FileNotFoundError:
        print(f"[WARN] Timestamps CSV not found: {csv_path}")
        return None
    except Exception as e:
        print(f"[WARN] Failed to read timestamps CSV {csv_path}: {e}")
        return None

    if legacy_rows:
        legacy_rows.sort(key=lambda t: t[0])
        offsets = [r[0] for r in legacy_rows]
        base_epoch = legacy_rows[0][2]
        return {"rows": legacy_rows, "offsets": offsets, "base_epoch": base_epoch, "mobile": False}

    if not mobile_rows:
        print(f"[WARN] No usable rows in timestamps CSV: {csv_path}")
        return None

    mobile_rows.sort(key=lambda t: t[0])
    deltas = [next_row[0] - curr_row[0] for curr_row, next_row in zip(mobile_rows[:-1], mobile_rows[1:]) if next_row[0] > curr_row[0]]
    default_delta = int(np.median(deltas)) if deltas else 0
    rows_derived = []
    for idx, (start, epoch, lat, lon, acc) in enumerate(mobile_rows):
        size = (mobile_rows[idx + 1][0] - start) if idx + 1 < len(mobile_rows) else default_delta
        if size <= 0:
            continue
        rows_derived.append((start, size, epoch, lat, lon, acc))

    if not rows_derived:
        print(f"[WARN] No usable rows in mobile timestamps CSV: {csv_path}")
        return None

    rows_derived.sort(key=lambda t: t[0])
    offsets = [r[0] for r in rows_derived]
    base_epoch = rows_derived[0][2]
    return {"rows": rows_derived, "offsets": offsets, "base_epoch": base_epoch, "mobile": True}


def _sample_index_to_timestamp(sample_index: int, fs_in: float, bytes_per_sample: int, chunk_ts_index, zero_ref: bool = True):
    """Map a sample index to epoch seconds using chunk timing metadata.

    When zero_ref is True, subtract the capture start epoch (legacy behavior).
    Returns NaN if mapping is not possible.
    """
    if chunk_ts_index is None or sample_index is None or sample_index < 0:
        return float("nan")
    if fs_in <= 0 or bytes_per_sample is None or bytes_per_sample <= 0:
        return float("nan")

    byte_pos = int(sample_index) * int(bytes_per_sample)
    offsets = chunk_ts_index.get("offsets", [])
    rows = chunk_ts_index.get("rows", [])

    if not offsets or not rows:
        return float("nan")

    idx = bisect_right(offsets, byte_pos) - 1
    if idx < 0 or idx >= len(rows):
        return float("nan")

    start_offset, chunk_size, epoch_seconds, *_rest = rows[idx]
    if byte_pos < start_offset or byte_pos >= start_offset + chunk_size:
        return float("nan")

    samples_into_chunk = (byte_pos - start_offset) / float(bytes_per_sample)
    base_epoch = chunk_ts_index.get("base_epoch", None)
    t = epoch_seconds + samples_into_chunk / float(fs_in)
    if zero_ref and base_epoch is not None:
        t -= base_epoch  # zero-reference to capture start
    return t


def sample_index_to_timestamp(sample_index: int, fs_in: float, bytes_per_sample: int, chunk_ts_index):
    return _sample_index_to_timestamp(sample_index, fs_in, bytes_per_sample, chunk_ts_index, zero_ref=True)


def sample_index_to_mobile_timestamp(sample_index: int, fs_in: float, bytes_per_sample: int, chunk_ts_index):
    """Return absolute epoch seconds from mobile timestamp metadata; NaN if unavailable."""
    return _sample_index_to_timestamp(sample_index, fs_in, bytes_per_sample, chunk_ts_index, zero_ref=False)


def sample_index_to_mobile_location(sample_index: int, fs_in: float, bytes_per_sample: int, chunk_ts_index):
    """Return (lat, lon, accuracy_m) for the chunk containing the sample; NaN triple if unavailable."""
    if chunk_ts_index is None or not chunk_ts_index.get("mobile", False):
    ...
    start_offset, chunk_size, epoch_seconds, lat, lon, acc = rows[idx]
    if byte_pos < start_offset or byte_pos >= start_offset + chunk_size:
        return float("nan"), float("nan"), float("nan")
    ...
    *,
    ...
    def _try_decode(bits: np.ndarray, aa_pos: int, aa_corr: int, pre_corr: int,
        ...
        for slip in slip_list:
            ...
            # Classify tag type from payload
            tag_type = classify_tag_type(payload.hex())
            
            # For Apple tags, detect variant and prepare ground truth
            ground_truth_apple = "0"
            if tag_type == "APPLE":
                ground_truth_apple, _ = detect_apple_variant_and_mask(payload.hex(), "")
            ...
            pkt = {
                ...
                "tag_type": tag_type,
                "ground_truth_apple": ground_truth_apple,
            }
...
def _mp_init_shared(freq_shm_name, freq_shape, freq_dtype_str,
    ...
    _MP_CHUNK_TS = chunk_ts_index
    _MP_BYTES_PER_SAMPLE = bytes_per_sample
...
def _process_one_window_task(task):
    ...
    need_pkt, need_cfo_row,
    chunk_ts_index, bytes_per_sample, hmac_key) = task

    if _MP_CHUNK_TS is not None:
        chunk_ts_index = _MP_CHUNK_TS
    if _MP_BYTES_PER_SAMPLE is not None:
        bytes_per_sample = _MP_BYTES_PER_SAMPLE
    ...
    if pkt_for_print.get("is_tag_ecosystem", False):
        ...
        if adva != "NO_AdvA":
            out["tag_adva"] = adva
    # Apply Samsung PRIVID conversion and HMAC-based MAC hiding (always, for any output)
    tag_type = pkt_for_print.get("tag_type", "UNKNOWN")
    payload_hex = pkt_for_print.get("payload_hex", "")
    original_mac = pkt_for_print.get("AdvA", "")
    
    # Apply Apple masking first if applicable
    if tag_type == "APPLE" and payload_hex:
        ground_truth, masked_mac = detect_apple_variant_and_mask(payload_hex, original_mac)
        if ground_truth != "0":
            pkt_for_print["AdvA"] = masked_mac
            pkt_for_print["ground_truth_apple"] = ground_truth
    
    # Apply Samsung PRIVID conversion if applicable
    if tag_type == "SAMSUNG" and payload_hex:
        privid = samsung_mac_to_privid(payload_hex)
        if privid:
            # Format 8-byte PRIVID with colons for HMAC processing: XX:XX:XX:XX:XX:XX:XX:XX
            privid_formatted = ":".join(privid[i:i+2] for i in range(0, len(privid), 2))
            pkt_for_print["AdvA"] = privid_formatted
    
    # Apply HMAC-based MAC hiding to all packets
    if original_mac and hmac_key is not None:
        current_mac = pkt_for_print.get("AdvA", original_mac)
        hidden_mac = hide_mac_with_hmac(current_mac, hmac_key)
        pkt_for_print["AdvA"] = hidden_mac
        ...
        if overall_cfo_hz is not None and np.isfinite(overall_cfo_hz):
            ...
            if need_cfo_row and out["is_tag_for_cfo"]:
                ...
                if aa_pos_i >= 0 and fs_in > 0:
                    ...
                    aa_start_orig = int(round(aa_start_up_global / scale))
                    timestamp_s = sample_index_to_timestamp(
                        aa_start_orig,
                        fs_in=fs_in,
                        bytes_per_sample=bytes_per_sample,
                        chunk_ts_index=chunk_ts_index,
                    )
                    mobile_timestamp = sample_index_to_mobile_timestamp(
                        aa_start_orig,
                        fs_in=fs_in,
                        bytes_per_sample=bytes_per_sample,
                        chunk_ts_index=chunk_ts_index,
                    )
                    mobile_lat, mobile_lon, mobile_acc = sample_index_to_mobile_location(
                        aa_start_orig,
                        fs_in=fs_in,
                        bytes_per_sample=bytes_per_sample,
                        chunk_ts_index=chunk_ts_index,
                    )
                    if not np.isfinite(timestamp_s) and fs_out > 0:
                        timestamp_s = aa_start_up_global / fs_out
                    if not np.isfinite(mobile_timestamp):
                        mobile_timestamp = float("nan")
                    if not np.isfinite(mobile_lat):
                        mobile_lat = float("nan")
                        mobile_lon = float("nan")
                        mobile_acc = float("nan")
                else:
                ...
                out["cfo_row"] = {
                    ...
                    "mobile_lon": mobile_lon,
                    "mobile_accuracy_m": mobile_acc,
                    "AdvA": adva,
                    ...
                    "tag_type": pkt_for_print.get("tag_type", "UNKNOWN"),
                    "ground_truth_apple": pkt_for_print.get("ground_truth_apple", "0"),
...
def ble_sniffer(
    ...
    # Generate random HMAC key for MAC hiding
    generate_hmac_key()
    print(f"[INFO] Generated random HMAC key for MAC address hiding")
    ...
    if use_mp:
        ...
        try:
            ...
            for idx, (s_up, e_up) in enumerate(windows):
                tasks.append((
                    ...
                    None, int(bytes_per_sample), _HMAC_KEY,
                ))
            ...
            with ProcessPoolExecutor(
                ...
                initargs=(
                    ...
                    chunk_ts_index, int(bytes_per_sample),
                ),
            ) as ex:
        ...
        for (s_up, e_up) in windows:
            ...
            # Apply Apple variant masking, Samsung PRIVID conversion, and HMAC-based MAC hiding (always, for any output)
            tag_type = pkt_for_print.get("tag_type", "UNKNOWN")
            payload_hex = pkt_for_print.get("payload_hex", "")
            original_mac = pkt_for_print.get("AdvA", "")
            ...
            # Apply HMAC-based MAC hiding to all packets
            if original_mac and _HMAC_KEY is not None:
                current_mac = pkt_for_print.get("AdvA", original_mac)
                hidden_mac = hide_mac_with_hmac(current_mac, _HMAC_KEY)
                pkt_for_print["AdvA"] = hidden_mac
                ...
                if overall_cfo_hz is not None and np.isfinite(overall_cfo_hz):
                    ...
                    if bool(pkt_for_print.get("is_tag_ecosystem", False)):
                        ...
                        if aa_pos_i >= 0:
                            ...
                            if not np.isfinite(timestamp_s) and fs_in > 0:
                                timestamp_s = float(aa_start_orig) / float(fs_in)
                            if not np.isfinite(mobile_timestamp):
                                mobile_timestamp = float("nan")
                            if not np.isfinite(mobile_lat):
                                mobile_lat = float("nan")
...
def main():
    ...
    ap.add_argument("--timestamps-csv", type=str, default=None,
                    help="Optional per-chunk timestamp CSV (chunk_index,byte_offset,chunk_size,epoch_seconds)")
    ...
    bytes_per_sample = 8 if ext in [".cf32", ".cfile"] else 4

    chunk_ts_index = load_chunk_timestamp_index(args.timestamps_csv) if args.timestamps_csv else None
    if chunk_ts_index:
        print(f"[INFO] Loaded chunk timestamps from {args.timestamps_csv} (rows={len(chunk_ts_index['rows'])})")
    elif args.timestamps_csv:
        print(f"[WARN] Using fallback sample-based timestamps; could not load {args.timestamps_csv}")

...
bool BLESDR::DecodeBTLEPacket(int32_t /*sample*/, int sps)
{
    ...
    // Preserve whitened payload bytes (before dewhitening)
    uint8_t payload_whitened[MAX_LE_SYMBOLS] = {0};
    int payload_whitened_len = packet_length;
    if (payload_whitened_len > 0) {
        const int copy_len = std::min(payload_whitened_len, (int)MAX_LE_SYMBOLS);
        std::memcpy(payload_whitened, packet_data + 2, (size_t)copy_len);
        payload_whitened_len = copy_len;
    }
    ...
    if (packet_crc == calced_crc) {
        ...
        // Store whitened payload bytes (as captured on-air)
        packet.payload_whitened_len = payload_whitened_len;
        if (payload_whitened_len > 0) {
            std::memcpy(packet.payload_whitened, payload_whitened, (size_t)payload_whitened_len);
        }

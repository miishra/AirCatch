...
struct lell_packet {
    ...
    // Raw whitened payload bytes (PDU payload only, Length bytes)
    uint8_t payload_whitened[MAX_LE_SYMBOLS];
    int payload_whitened_len = 0;
...
public:
    ...
    void set_sample_rate(double sr_hz) {
        sample_rate_hz_ = (sr_hz > 0.0) ? sr_hz : 2e6;
    }

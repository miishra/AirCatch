...
# FNAME = "cfo_samples_rail_privid.csv"
...
# Input capture and output folder. Both default to this artifact's own files and
# can be overridden on the command line:
#     python3 fingerprint_classifier.py [input.csv] [output_dir]
_HERE = os.path.dirname(os.path.abspath(__file__))
FNAME = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    _HERE, "dataset", "blephasyr_static_devices.csv")
OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
    _HERE, "fingerprint_out")

# NOTE: resolved at runtime to "AdvA" or "adv_addr" (or case-insensitive match)
MAC_COL = "AdvA"

TRAIN_FRACTION = 0.8          # temporal split within each MAC
MIN_PKTS_PER_MAC = 200        # minimum usable packets per MAC (after dropping NaNs) to keep class
...
MEAN_REPEATS = 1              # mean repeated this many times per CFO column (>=2 strongly biases RF toward mean)

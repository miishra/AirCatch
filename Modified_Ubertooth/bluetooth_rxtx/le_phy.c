...
static int8_t mean_i8_round_clip(int32_t s, uint32_t n)
{
	if (n == 0) return 0;

	// round to nearest integer
	// for negative numbers, subtract n/2 before division to round away from 0 correctly
	int32_t q = (s >= 0) ? (s + (int32_t)(n / 2)) : (s - (int32_t)(n / 2));
	q /= (int32_t)n;
...
typedef struct _le_rx_t {
	...
	int8_t  freqest_per_byte[2 + 255 + 3];
...
static void compute_transition_cfo_features(le_rx_t *pkt);
...
static void buffer_clear(le_rx_t *buf) {
	...
	buf->rssi_min = 0; // INT8_MAX;
	buf->rssi_max = 0; // INT8_MIN;
	buf->rssi_sum = 0;
	...
	// Avoid div0 in usb_enqueue_le(); we’ll repurpose rssi_* fields for CFO features.
    buf->rssi_cnt = 1;

	// NEW
	memset(buf->freqest_per_byte, 0, sizeof(buf->freqest_per_byte));
}
...
void le_DMA_IRQHandler(void) {
	...
	if (DMACIntStat & (1 << 0)) {
		...
		if (DMACIntTCStat & (1 << 0)) {
			...
			current_rxbuf->freqest_per_byte[pos] = offset;
...
static int usb_enqueue_le(le_rx_t *packet) {
	...
	// NOTE: we keep existing expression, but we will *overwrite offset_sum*
    // in compute_transition_cfo_features() so that offset_avg becomes CFO(total).
    f->offset_avg = packet->offset_sum / (int) (packet->size - packet->rssi_cnt);
...
// NEW: compute “transition CFOs” using per-byte FREQEST samples.
// Output is packed into existing fields so we don't change USB protocol:
//   rssi_avg  := CFO(00)  (int8, FREQEST LSB)
//   rssi_min  := CFO(11)
//   rssi_max  := CFO(10)
//   offset_min:= CFO(01)
//   offset_avg:= CFO(total)  (through offset_sum trick)
static void compute_transition_cfo_features(le_rx_t *pkt) {
	const unsigned total_bytes = pkt->size;
	const unsigned total_bits = total_bytes * 8;
	if (total_bits < 2) return;

	int32_t s00=0, s11=0, s10=0, s01=0, st=0;
	uint32_t n00=0, n11=0, n10=0, n01=0, nt=0;

#define GET_BIT(i) (((pkt->data[(i)>>3] >> ((i)&7)) & 1))
	for (unsigned i = 1; i < total_bits; i++) {
		const unsigned b0 = GET_BIT(i-1);
		const unsigned b1 = GET_BIT(i);
		const unsigned byte_idx = (i >> 3);
		if (byte_idx >= total_bytes) break;

		const int8_t fe = pkt->freqest_per_byte[byte_idx]; // FREQEST LSB units
		st += fe; nt++;
		if (b0==0 && b1==0) { s00 += fe; n00++; }
		else if (b0==1 && b1==1) { s11 += fe; n11++; }
		else if (b0==1 && b1==0) { s10 += fe; n10++; }
		else { s01 += fe; n01++; }
	}
#undef GET_BIT

	const int8_t c00 = mean_i8_round_clip(s00, n00);
	const int8_t c11 = mean_i8_round_clip(s11, n11);
	const int8_t c10 = mean_i8_round_clip(s10, n10);
	const int8_t c01 = mean_i8_round_clip(s01, n01);
	const int8_t cT  = mean_i8_round_clip(st,  nt);

	// Pack into existing fields (single LE_PACKET only)
	pkt->rssi_cnt = 1;
	pkt->rssi_sum = c00;   // rssi_avg = rssi_sum / rssi_cnt
	pkt->rssi_min = c11;
	pkt->rssi_max = c10;
	pkt->offset_min = c01;

	// Make usb_enqueue_le() compute offset_avg == cT:
	// offset_avg = offset_sum / (size - rssi_cnt)
	const int denom = (int)pkt->size - (int)pkt->rssi_cnt;
	pkt->offset_sum = (denom > 0) ? ((int)cT * denom) : 0;
}
...
// Fixed mapping: always present, no packet-length dependency:
//   rssi_avg   := cTot
//   rssi_min   := cW[0]
//   rssi_max   := cW[8]
//   offset_min := cW[4]
//   offset_avg := cW[2]
static void compute_hweight_cfo_bins(le_rx_t *pkt)
{
	const unsigned total_bytes = pkt->size;
	...
	// Choose what bytes to include:
	// If you want to exclude header+CRC, use:
	// unsigned b0 = 2;
	// unsigned b1 = (pkt->size >= 3) ? (pkt->size - 3) : pkt->size;
	unsigned b0 = 0;
	unsigned b1 = total_bytes;

	for (unsigned b = b0; b < b1; b++) {
		const uint8_t byte = pkt->data[b];
		const uint8_t w = popcount8(byte);               // 0..8
		const int8_t  fe = pkt->freqest_per_byte[b];     // int8, FREQEST LSB units
	...
	const int8_t cTot = mean_i8_round_clip(sum_all, cnt_all);
	const int8_t cW0  = mean_i8_round_clip(sum_w[0], cnt_w[0]);
	const int8_t cW2  = mean_i8_round_clip(sum_w[2], cnt_w[2]);
	const int8_t cW4  = mean_i8_round_clip(sum_w[4], cnt_w[4]);
	const int8_t cW8  = mean_i8_round_clip(sum_w[8], cnt_w[8]);

	// Map into existing fields (these are what usb_enqueue_le() exports)
	pkt->rssi_cnt   = 1;
	pkt->rssi_sum   = cTot;  // rssi_avg = rssi_sum / rssi_cnt
	pkt->rssi_min   = cW0;
	pkt->rssi_max   = cW8;
	pkt->offset_min = cW4;

	// usb_enqueue_le computes offset_avg = offset_sum / (size - rssi_cnt)
	// Make that equal to cW2
	const int denom = (int)pkt->size - (int)pkt->rssi_cnt;
	pkt->offset_sum = (denom > 0) ? ((int)cW2 * denom) : 0;
}
	...
	while (requested_mode == MODE_BT_FOLLOW_LE) {
		...
		if (queue_remove(&packet_queue, (void **)&packet)) {
			...
			if (filter_match(packet)) {
				...
				// NEW: fill CFO fields (reusing rssi/offset fields)
				// before usb_enqueue_le copies them to USB.
				// compute_transition_cfo_features(packet);

				compute_hweight_cfo_bins(packet);
